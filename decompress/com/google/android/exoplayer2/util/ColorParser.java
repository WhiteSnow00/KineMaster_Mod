...
public final class ColorParser
{
    ...
    private static int a(final String s, final boolean b) {
        Assertions.a(TextUtils.isEmpty((CharSequence)s) ^ true);
        ...
        if (replace.startsWith("rgba")) {
            Pattern pattern;
            if (b) {
                pattern = ColorParser.c;
            }
            else {
                pattern = ColorParser.b;
            }
            final Matcher matcher = pattern.matcher(replace);
            if (matcher.matches()) {
                int int1;
                if (b) {
                    int1 = (int)(Float.parseFloat(Assertions.e(matcher.group(4))) * 255.0f);
                }
                else {
                    int1 = Integer.parseInt(Assertions.e(matcher.group(4)), 10);
                }
                return Color.argb(int1, Integer.parseInt(Assertions.e(matcher.group(1)), 10), Integer.parseInt(Assertions.e(matcher.group(2)), 10), Integer.parseInt(Assertions.e(matcher.group(3)), 10));
            }
        }
        else if (replace.startsWith("rgb")) {
            final Matcher matcher2 = ColorParser.a.matcher(replace);
            if (matcher2.matches()) {
                return Color.rgb(Integer.parseInt(Assertions.e(matcher2.group(1)), 10), Integer.parseInt(Assertions.e(matcher2.group(2)), 10), Integer.parseInt(Assertions.e(matcher2.group(3)), 10));
            }
        }
        else {
            final Integer n3 = ColorParser.d.get(Ascii.e(replace));
            if (n3 != null) {
                return n3;
            }
        }
